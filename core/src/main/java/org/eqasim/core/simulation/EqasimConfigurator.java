...
import org.eqasim.core.simulation.termination.market_error.TerminationMarketErrorModule;
...
public abstract class EqasimConfigurator {
	...
	public void updateConfig(Config config) {
		...
		// Configure all output intervals to only write on the last iteration
		configureOutputIntervalsForLastIterationOnly(config);
	}

	/**
	 * Configures all MATSim output intervals to only write on the last iteration.
	 * This prevents the creation of multiple iteration output files in the ITERS directory
	 * and ensures that only the final iteration outputs are stored.
	 * 
	 * @param config The MATSim configuration to modify
	 */
	private void configureOutputIntervalsForLastIterationOnly(Config config) {
		int lastIteration = config.controller().getLastIteration();
		
		// Configure controller output intervals to only write on last iteration
		// Use a value larger than the total number of iterations to ensure files are only written at the end
		int largeInterval = lastIteration + 1;
		
		// Core MATSim output intervals
		config.controller().setWriteEventsInterval(largeInterval);
		config.controller().setWritePlansInterval(largeInterval);
		config.controller().setWriteSnapshotsInterval(largeInterval);
		config.controller().setWriteTripsInterval(largeInterval);
		config.controller().setCreateGraphsInterval(largeInterval);
		
		// Note: Graph creation is controlled by createGraphsInterval above
		
		// Configure DiscreteModeChoice output intervals
		if (config.getModules().containsKey(DiscreteModeChoiceConfigGroup.GROUP_NAME)) {
			...
			dmcConfig.setWriteUtilitiesInterval(largeInterval);
		}
		
		// Configure counts analysis intervals
		config.counts().setWriteCountsInterval(largeInterval);
		
		// Configure link stats intervals  
		config.linkStats().setWriteLinkStatsInterval(largeInterval);
		
		// Configure additional MATSim analysis intervals that generate iteration-specific files
		// These control legHistogram.txt, legDurations.txt and other built-in MATSim analyses
		
		// Set analysis intervals to only run on last iteration
		// Note: Some of these might not be available in all MATSim versions, so we use try-catch
		try {
			// Configure scoring intervals
			config.scoring().setWriteExperiencedPlans(false); // Don't write experienced plans every iteration
		} catch (Exception e) {
			// Ignore if not available in this MATSim version
		}
		
		// For remaining MATSim built-in analysis outputs, they are typically controlled by
		// the main output intervals we've already set above
		
		logger.info("Configured all output intervals to only write on the last iteration (iteration {})", lastIteration);
	}
